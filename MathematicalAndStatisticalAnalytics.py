...
4. Class for OLS model performance metrics.
5. Class for statistics and statistical testing.


NumPy is needed for this package. Most functions require pandas objects to be converted
to lists or numpy arrays prior to passing through the function.
...
class OLSLinAlg:
    ...
    def GetMatrix(rows, columns, normalMean=0, normalStd=1):
        ...
        normalStd: Integer or float value for the standard deviation of the randomly generated variable that 
                   follows a normal distribution. The default value is 1.
    ...
    def GetMatrixInverse(matrix1):
        ...
        for row in range(len(matrix2)):
            ...
            for col in range(len(matrix2)):
                minor = OLSLinAlg.GetMatrixMinor(matrix2,row,col)
                cofactorRow.append(((-1)**(row+col)) * OLSLinAlg.GetDeterminant(minor))
...
class OLSSuite:
    ...
    def GetPredictions(self, x):
        ...
        x: Independent variables to pass through the model. Can pass a numpy array or list object. 
    ...
    def GetCoefStdErrors(self, y, x):
        ...
        mse = Metrics.GetMSE(y, self.GetPredictions(x))
    ...
    def GetOLSResults(self, y, x, xVariables=[]):
        ...
        self.R2Score = Metrics.GetR2(y, self.GetPredictions(x))
        self.AdjR2Score = Metrics.GetAdjustedR2(self.R2Score, len(x), len(x[0]))
    ...
    def GetAltOLS(self, y, x, xVariables=[], R2PerformanceThreshold = .1):
        ...
        elif type(xVariables) != list:
            xVariables = list(xVariables)
        else:
            xVariables = xVariables
...
class Metrics:
    '''This is a stateless class that has regression metrics.
    '''
...
class Stats:
    '''This is a stateless class that has statistics and tests.
    '''
    ...
    def Get1SampleTTest(x, popMean):
        ...
        t = (Mathematics.GetMean(x) - popMean) / (Mathematics.GetSquareRoot(Stats.GetVariance(x))/Mathematics.GetSquareRoot(len(x)))
    ...
    def GetTwoSampleTTest(x1, x2, xVariables=[], equalVariance=True):
        ...
        if equalVariance == True:
            pooledSE = Mathematics.GetSquareRoot((((len(x1)-1)*Stats.GetVariance(x1))+((len(x2)-1)*Stats.GetVariance(x2)))/(len(x1)+len(x2)-2))
            t = (Mathematics.GetMean(x1) - Mathematics.GetMean(x2))/(pooledSE*Mathematics.GetSquareRoot(1/len(x1)+1/len(x2)))
        else:
            t = (Mathematics.GetMean(x1) - Mathematics.GetMean(x2))/Mathematics.GetSquareRoot((Stats.GetVariance(x1)/len(x1))+(Stats.GetVariance(x2)/len(x2)))
