...
Please check function documentation for each function to check array dimensions. This package is written in
Cython, primarily relying on variable declaration and memory views for efficiency gains. Set up file is 
included and needed to run this package.
...
class OLSSuite:
    ...
    leveraging the ndarray object for matrix operations. All objects passed through this function must be
    2d numpy arrays. Functions are all written in Cython for more efficient matrix operations. 
    ...
    stdErrorCoefs: The standard errors for the model coefficients.

    R2Score: R2 score of the model.
    
    AdjR2Score: Adjusted R2 score of the model.

    dofResiduals: The degrees of freedom for the residuals of the model.

    nObservations: The number of observations for the OLS model.

    meanAbsoluteError: The mean absolute error for the OLS model.
    '''
    ...
    def GetCoefs(self, fusedDType[:, :] y, fusedDType2[:, :] x):
        ...
        y: A 2d numpy array of the response variable for the model. Please convert list of list or 
           Dataframes to numpy arrays before passing objects through the model.

        x: A 2d numpy array of the independent variables for the model. Please convert list of list
           or Dataframes to numpy arrays before passing objects through the model.

        Returns: The Coefficients attribute that is a 2d numpy array from the fitted model object.
        '''
    ...
    def GetPredictions(self, fusedDType[:, :] x):
        '''This function calculates the predictions for a given dataset. The model must be fit before
        running this function.
    
        Parameters:

        x: A 2d numpy array of the independent variables for the model. Please convert list of list or 
           Dataframes to numpy arrays before passing objects through the model.
    
        Returns: The standard error of the coefficients attribute that is a 2d numpy array from the
                 fitted model object.
        '''
    ...
    def GetOLSResults(self, fusedDType[:, :] y, fusedDType2[:, :] x, xVariables=[]):
        ...
        xVariables: List of the independent variable names in the order they were passed into the model.
                    If no value is passed, arbitrary names of x0 through number of variables is used.
    ...
    def GetAltOLS(self, fusedDType[:, :] y, fusedDType2[:, :] x, xVariables=[], double R2PerformanceThreshold = .1):
        ...
        R2PerformanceThreshold: A float value that is the minimum increase in R2 a linear transformation must 
                                achieve to be chosen over the default selection of a linear relationship.
        ...
        for i in range(xMatrix.shape[1]):
            ...
            if maxValue - R2Benchmark > R2PerformanceThreshold:
                if maxKey == 'square':
                    altXMatrix = np.append(altXMatrix, LinearTransformationDict[maxKey](np.reshape(altXMatrix[:, i], (-1, 1))), 1)
        ...
        self.GetOLSResults(yMatrix, altXMatrix, xVariables)
...
def GetMSE(fusedDType[:, :] yTrue, fusedDType2[:, :] yPred):
    ...
    yPred: A 2d numpy array containing the predicted y values that were used to train the OLS model.
...
def GetAdjustedR2(double r2, double n, double p):
    ...
    r2: The R2 score from the model. Must be of float datatype
...
def GetVariance(fusedDType[:] numbers):
    ...
    numbers: A 2d array of the data to calculate the variance for. Values can be of float or 
             int datatypes.
...
def GetTwoSampleTTest(fusedDType[:] x1, fusedDType2[:] x2, xVariables=[], equalVariance=True):
    ...
    xVariables: List of the independent variable names in the order they were passed into the model.
                If no value is passed, arbitrary names of x1 through number of variables is used.
