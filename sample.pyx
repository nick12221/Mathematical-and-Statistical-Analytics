...
def GetMatrixDiagonal(fusedDType[:,:] matrix1):
        ...
        matrix1 = floatConverter(matrix1)

        cdef Py_ssize_t i
        cdef Py_ssize_t z = 0

        diagonalMatrix = np.zeros((1,MatrixCols),dtype=dtype) 
        cdef fusedDType[:, :] resultView = diagonalMatrix

        for i in range(MatrixCols):
            resultView[z, i] = matrix1[i, i] 
...
def GetDeterminant(fusedDType[:,:] matrix1):
        ...
        cdef fusedDType[:, :] matrix2
        matrix2 = np.zeros((matrixLength, matrixLength), dtype=dtype)

        matrix2[:,:] = matrix1
        
        matrix1 = floatConverter(matrix1)
        matrix2 = floatConverter(matrix2)

        cdef Py_ssize_t i, j, k, x, newRow
        cdef fusedDType rowChange
        cdef fusedDType rowScaler
        cdef fusedDType changeIndicator
        cdef fusedDType determinant
        ...
        if sum([sub[0] for sub in matrix2]) == 0:
            ...
            if matrix2[0, 0] == 0:
                ...
                matrix2[0], matrix2[newRow] = matrix2[newRow], matrix2[0]
                rowChange = 1
...
def GetMatrixMinor(fusedDType[:,:] matrix1, Py_ssize_t i, Py_ssize_t j):
...
def GetMatrixInverse(fusedDType[:, :] matrix1):
    ...
    cdef fusedDType[:, :] identityMatrix
    identityMatrix = GetIdentityMatrix(matrixLength)

    cofactorMatrix = np.zeros((matrixLength, matrixLength), dtype=dtype) 
    cdef fusedDType[:, :] cofactors = cofactorMatrix

    matrix2 = np.zeros((matrixLength, matrixLength), dtype=dtype)
    cdef fusedDType[:, :] resultView = matrix2
    ...
    cdef fusedDType negNumber, posNumber
    negNumber = -1
    posNumber = 1
...
class OLSSuite:
    ...
    def GetCoefs(self, fusedDType[:, :] y, fusedDType[:, :] x):
        ...
        self.Coefficients = GetDotProduct(GetDotProduct(GetMatrixInverse((GetDotProduct(xTransposed, x))),xTransposed),y)
    ...
    def GetPredictions(self, fusedDType[:, :] x):
        ...
        preds = GetDotProduct(GetTransposeMatrix(self.Coefficients), GetTransposeMatrix(x))
        preds = GetTransposeMatrix(preds)
    
        return preds


    def GetCoefStdErrors(self, fusedDType[:, :] y, fusedDType[:, :] x):
        ...
        mse = GetMSE(y, self.GetPredictions(x))
        self.stdErrorCoefs = (mse*diagonalMatrix)**.5


    def GetOLSResults(self, fusedDType[:, :] y, fusedDType[:, :] x, xVariables=[]):
        ...
        self.R2Score = GetR2(y, self.GetPredictions(x))
        self.AdjR2Score = GetAdjustedR2(self.R2Score, len(x), len(x[0]))
        self.stdErrorCoefs = self.GetCoefStdErrors(y, x)
        self.dofResiduals = len(x) - len(x[0])
...
def GetMSE(fusedDType[:, :] yTrue, fusedDType[:, :] yPred):
    ...
    squaredError = np.zeros((yTrue.shape[0], yTrue.shape[1]), dtype=dtype) 
    cdef fusedDType[:, :] resultView = squaredError

    for i in range(yTrue.shape[0]):
        resultView[i, j] = (yTrue[i, j] - yPred[i, j])**2

    MSE = sum(squaredError)/yTrue.shape[0]

    return MSE


def GetR2(fusedDType[:, :] yTrue, fusedDType[:, :] yPred):
    ...
    cdef fusedDType r2, yMean, posOne
    cdef Py_ssize_t i, j
    j = 0
    posOne = 1
    ...
    totalVariability = np.zeros((yTrue.shape[0], yTrue.shape[1]), dtype=dtype) 
    cdef fusedDType[:, :] resultView2 = totalVariability
