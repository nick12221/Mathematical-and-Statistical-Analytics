...
for i in data['Sex_category'].unique():
    ...
    print(f'{i} confidence interval {get_confidence_interval(df2, "BirthWeightOz", ci=.95)}')
...
#Exponentiate the coefficients to get the true percentage change
coeff_df['Exp. Coefficient'] = np.where(coeff_df['Variable'] == 'const', np.exp(coeff_df['Coefficient']), np.exp(coeff_df['Coefficient']) - 1)
coeff_df
